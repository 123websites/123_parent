...
class FusionTableHandler{
	...
	private $geometry_locations = [2, 4, 0];
	...
	public function build_sql_tables(){
		foreach($this->sql_table_names as $index => $sql_table_name){
			if( $this->sql_table_exists($sql_table_name) == false ){
				...
				$fusion_table_data = $this->get_fusion_table_data($this->table_names[$index], $this->pair_delimiters[$index], $this->geometry_locations[$index]);
	...
	public function get_fusion_table_data($table_name, $pair_delimiter, $geometry_location){
		// get lines
		$csv = [];
		...
		foreach($csv as $index => $content){
			...
			if( $index == 0 ){
				...
				if( !empty($content) ){
					if( !isset($content[$geometry_location]) ){
						continue;
					}

					// create xml element
					$xml = new SimpleXMLElement($content[$geometry_location]);
					...
					$content[$geometry_location] = $all_coord_pairs;
...
// error_log(print_r($fth->get_fusion_table_data('countries', ',0', 0), true));
