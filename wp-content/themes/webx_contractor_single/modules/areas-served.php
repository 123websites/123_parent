...
<main class="areas-served main<?php echo $has_bg ? ' main--hasbg' : ''; ?>" id="areas-served" <?php if($has_bg): ?> style="background-image: url('<?php echo get_field('areas-served-bg', 'option'); ?>');" <?php endif; ?>>
	<section class="areas-served-hero hero"">
		<div class="areas-served-hero-text hero-text">
			<h1 class="areas-served-hero-text-header hero-text-header<?php echo !$has_bg ? ' hero-text-header--nobg' : ''; ?>"><?php echo get_field('areas-served-alt-toggle', 'option') ? get_field('areas-served-alt', 'option') : 'areas served' ?></h1>
		</div>
		...
		<?php if(have_rows('locations', 'option')) : ?>
			<div class="areas-served-areas-grid">
				...
				foreach($rows as $index => $row): 
					...
					<a href="https://www.google.com/maps/@<?php echo $contents->result->geometry->location->lat . ',' . $contents->result->geometry->location->lng . ',14z'; ?>" class="areas-served-areas-grid-imagecontainer" target="_blank">
